...
```{r}
lme1 <- nlme::lme(sqrt_aort_grad ~ oper + sex + tstart, 
                  random = (~ 1 + tstart | id),
                  data = aort1)
summary(lme1)
tidy(lme1)
...
model	term	       2.5 %    	97.5 %
two-time	operSI	-0.6356173	-0.0966928
boots	operSI    	-0.7919904	-0.2997002
two-time	sexMale	-0.3683776	0.1460658
boots	sexMale	    -0.2161467	0.2858354
two-time	sqrt_ao  0.2645691	0.4400034
boots	sqrt_aort_   0.2718825	0.4435482
...
Hence, there will be a bias for the implementation that remove all the measurement errors for unbiased predictors, which produce biased results by empirical Bayesian estimates from random effects.
...
boot1 <- map_df(.x = 1:3, ~ get_boot(cox_boots, index = .x)) %>%
